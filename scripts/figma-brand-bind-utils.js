 ...
 *
 * Copiar este bloque en scripts Brand (figma_execute no puede importar archivos).
 */
...
function bindFill(node, variable, opacity, collections, allVars) {
...
function paintVar(variable, opacity, collections, allVars, node) {
  const color = resolveVariableColor(variable, collections, allVars, node);
  const base = { type: 'SOLID', color, visible: true };
  if (opacity !== undefined && opacity < 1) base.opacity = opacity;
